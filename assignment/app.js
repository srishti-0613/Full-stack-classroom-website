...
const mongoURI = "mongodb+srv://srishti325:2515@database-5ivou.mongodb.net/test?retryWrites=true&w=majority";
...
app.get("/create/:cid/", async (req, res) => {
    try{
       res.render("./teacher/teacher_assign_create", { cid: req.params.cid });
    }
    catch(e){
        console.log("Error" , e);
    }
...
app.get("/assign-show/:cid/", async (req, res) => {
    try {
        ...
        console.log(ad);
        res.render("./teacher/teacher_classwork.ejs", { data: ad, id: id  , cid : cid});
    }
    catch (e) {
        console.log("Error", e);
    }
...
app.get("/create-class/:id/", async (req, res) => {
    try{
        ...
        var name = data.name;
        res.render("./teacher/class_creation" , {id : id , name : name});
    }
    catch(e){
        console.log("Error", e);
    }
...
app.post("/create-class/:id/", async (req, res) => {
    ...
    try {
        ...
        const g = await teacher.create({
            fc_name: data[0].name,
            fc_id: data[0]._id,
        });
        const ifFound = await dclass.find({ joining_id: req.body.joining_id });
        if (ifFound != '') {
            res.render("./teacher/class_creation", { id: req.params.id  ,name : data[0].name });
        }
        else {
...
app.get("/show-students/:cid/", async (req, res) => {
    try {
        ...
        res.render("./teacher/teacher_students", { data: data.students, cid: cid , id : id  , fc_name : name});
    }
    catch (e) {
       console.log("Error" , e);
    }
    //res.render("./teacher/teacher_students.ejs");
});
...
app.get("/show-classes/:id/", async (req, res) => {
    ...
    const s_classes = await dclass.find({ st_id:
        ...
        //res.send("vuk");
    res.render("./teacher/classes.ejs" , {  s_classes : s_classes  , f_classes: f_classes  , id :id});
    }
    catch(e){
        console.log("Error" , e);
    }
...
app.get("/Show-submitted-hw/:aid/:cid/" , async (req , res)=>{
    ...
    try{
        ...
        res.render("./teacher/teacher_submitted_hw" , {data : hdata , cid : cid , aid : aid , id:id , title : name});
    }
    catch(e){
     console.log("Error" , e)   
    }
